...
class DirectImport
  ...
  def import_events
    puts "Direct importing events from: #{@file_path}"
    ...
    begin
      ...
      (2..sheet.last_row).each do |row|
        begin
          ...
          sql = <<-SQL
            INSERT INTO eventos (
              tipo_evento, fluxo, valor, responsavel, data_evento, 
              descricao, status, created_at, updated_at,
              veiculo_id, cliente_id, created_by, updated_by
            ) VALUES (
          ...
          ActiveRecord::Base.connection.execute(
            ...
            tipo_evento_db,
            fluxo_db,
            evento_data[:valor_pago] || 0,
            evento_data[:responsavel_pagamento] || 'Sistema',
            evento_data[:data] || Date.current,
            evento_data[:descricao],
            'pago',
            Time.current,
            Time.current,
            veiculo_id,
...
if __FILE__ == $0
  ...
  importer = DirectImport.new(file_path)
